...
function calcAvgCartCoord(points) {
    let sumX = 0;
    let sumY = 0;
    let sumZ = 0;
...
function convCartToLatLon(x, y, z) {
...
function calcMidPoint(coord) {
    ...
    const avgCart = calcAvgCartCoord(cartPoints);

    // Convert the average Cartesian coordinates back to latitude and longitude.
    const avgDeg = convCartToLatLon(avgCart.x, avgCart.y, avgCart.z);
